...
export function useLocalStorage<T = string>({
	...
	const [value, setValue] = useState<T>(
		typeof window !== "undefined" &&
			...
			? deserialize((window as any).localStorage.getItem(key) ?? undefined)
			: ((defaultValue ?? "") as T)
	);
		...
		(val: T | ((prevState: T) => T)) => {
			if (val instanceof Function) {
				setValue((current) => {
					...
					(window as any).localStorage.setItem(key, serialize(result));
				...
				(window as any).localStorage.setItem(key, serialize(val));
				(window as any).dispatchEvent(
					new CustomEvent("mantine-local-storage", {
	...
	useWindowEvent("storage", (event) => {
		if (
			event.storageArea === (window as any).localStorage &&
			event.key === key
		) {
